...
def pos_embeddings(inputs, emb_dim, max_length=2000):
  ...
  sequence = tf.concat([tf.range(max_length), tf.fill([20000], 0)], axis=-1)
  sequence = tf.slice(sequence, [0], [T])
  position_ind = tf.tile(tf.expand_dims(sequence, 0), [N, 1])
  pos_embeddings = tf.nn.embedding_lookup(lookup_table, position_ind)
  return inputs + pos_embeddings

def attention(queries, keys, num_heads, residual='add', use_pos_embeddings=True, queries_eq_keys=False, training=False):
