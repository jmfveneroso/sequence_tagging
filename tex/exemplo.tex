...
\begin{equation}
\mathcal{L}(Y; \theta) = \prod_{i=1}^{n} P(y_i|y_{i-1})
\mathcal{L}(Y; \theta) = \prod_{i=1}^{n} \theta_{y_i, y_{i-1}}
\end{equation}

Finally, to obtain maximum likelihood 
esimates for the transition matrix parameters, it is useful to define the identity function
$ I(x) $ defined in Equation~\ref{eq:identity_fn}.

\begin{equation}
I(x) = \begin{cases}
    1, & \text{if the statement } x \text{is true} \\
    0, & \text{otherwise}
\end{cases}
\label{eq:identity_fn}
\end{equation}

That way we can write the likelihood $ \mathcal{L(Y; \theta)} $ as:

\begin{equation}
\mathcal{L}(Y; \theta) = \prod_{i=1}^{n} \prod_{a,b} \theta_{a, b}^{I(x_{i-1}=a, x_{i}=b)} (1 - \theta_{a, b})^{I(x_{i-1} \neq a, x_{i} \neq b)}
\label{eq:identity_fn}
\end{equation}

If we define the function $ K(a, b, X) $ to be equal to the count of transitions from $ a $ to $ b $
in sequence $ X $, we can write $ \mathcal{L}(Y; \theta) $ as:

\begin{equation}
\mathcal{L}(Y; \theta) = \prod_{a,b} \theta_{a, b}^K(a, b, X) (1 - \theta_{a, b})^{n - K(a, b, X)}
\mathcal{l}(Y; \theta) = \sum_{a,b} K(a, b, X) log \theta_{a, b} + (n - K(a, b, X)) \cdot (1 - \theta_{a, b})
\end{equation}

Since the likelihood function $ \mathcal{L}(Y; \theta) $ is always positive, maximizing the logarithm
of this function $ \mathcal{l}(Y; \theta) $ is identical to maximizing the function itself, with the
benefit of transforming a product over transition probabilities into the sum of the logarithm of 
the transition probabilities.

\begin{equation}
\frac{\partial \mathcal{l}(Y; \theta)}{\partial \theta_{a, b}} = \frac{K(a, b, X)}{\theta_{a, b}} - \frac{n - K(a, b, X)}{1 - \theta_{a, b}}
\end{equation}

Making $ \frac{\partial\mathcal{l}(Y; \theta)}{\theta_{a, b}} = 0 $ to find the maximum value of
$ \mathcal{l}(Y; \theta) $ and isolating $ theta_{a, b} $ yields a closed form expression to the 
...
What is really convenient, since this is average number of transitions from state $ a $ to state
$ b $ in sequence $ X $. A value that can be easily obtained with $ O(n) $ time complexity. Up to 
this point, we can easily estimate the transition matrix from the data, and we are also able to 
reliably predict the future states of a sequence based on the current state. But in NER, the words 
are observed, however the Named Entity labels associated with these words are not. That is why need
...
Since we will only be working with boolean features, these probability functions are simply 
identity functions.
...
That is, the probability of observing the word $ x_i $ depends only on the current hidden
state $ y_i $, its label (e.g. PER, LOC, ORG, etc.). So, by making this assumption, we are
essentialy stating that if we know a token's label, we can reliably predict what are the 
probabilities that this token takes any value in the vocabulary. This is, of course, a very
simplistic assumption and many models (such as CRFs) try to overcome this issue, but this 
step is necessary if we want to get a closed form estimator for our generative model. So,
we now introduce the \textit{Emission Matrix}...


With Bayes theorem, we can write the probability $ P(Y|X) $ as:
