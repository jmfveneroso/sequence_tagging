...
\begin{itemize}
\item \textbf{Labeled \gls{rne} dataset:} we introduce a labeled \gls{ner}
...
\begin{itemize}
\item \textbf{Chapter 2: Problem Definition} defines the problem of \gls{rne}
...
improvements to the models described in Chapter 4 to handle \gls{ner}
on HTML documents.

\item \textbf{Chapter 6: Researcher Name Extraction Dataset} presents the \gls{rne}
...
with the models from Chapter 4 and 5 in the \gls{rne} dataset introduced in 
Chapter 6.
...
It is different from \gls{ie} in plain text because HTML documents frequently
...
\gls{ie} in the context of the Message Understanding 
...
\gls{ner} in the web by assessing their performance in the \gls{rne} task.
...
The \gls{rne} task is a problem of \gls{wde} that
we solved with machine learning models for \gls{ner}.
