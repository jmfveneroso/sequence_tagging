...
methods for to extract bibliometric information such as authors, titles, affiliations, 
...
\chapter{Researcher Name Dataset}
\label{cha:dataset}

This chapter describes the dataset used to evaluate the performance of sequence models
on the \gls{wde} task of researcher name extraction. We call it the Researcher Name dataset.
We attested experimentally that 
models trained in a popular \gls{ner} dataset (the {CoNLL-2003} English dataset) do not perform 
well in the researcher name extraction task and this is likely due to the differences
between plain text and HTML, as we will discuss in this section. The lack of other \gls{ner} 
datasets built for HTML name extraction imposed the need to construct the Researcher Name dataset.

The researcher name extraction task consists of finding researcher names in faculty listings from university 
...
\gls{wde} datasets, each webpage in the dataset comes from a different 
...
The dataset size was limited to a number of pages that could represent a large portion of 
countries while still being viable to label manually. Also, university pages that contained 
corrupt HTML or JavaScript code that performed lazy loading of researcher data records were
...
of relevant named entities for this dataset, and discusses some baselines. Finally,
Section~\ref{sec:conll_comparison} compares the characteristics of the Researcher Name dataset
with another popular \gls{ner} dataset, the {CoNLL-2003} English NER dataset.
...
adopted here has proved to be sufficient for the researcher name dataset. 
...
The Researcher Name dataset only has entities of type person PER, therefore a classifier has to
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Description of the data files in the Researcher Name dataset}
  \label{tab:dataset}
\end{table}
...
Researcher Name dataset with exact and partial match tags. That is, if a sequence of tokens
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{DBLP dictionary coverage in each data file of the Researcher Name dataset.}
  \label{tab:gazetteer}
\end{table}

To understand how this dictionary can be useful for some models in the researcher name extraction task, 
we consider the Precision, Recall and $ F_1 $-scores for a exact dictionary matching 
strategy in each Researcher Name data file. The results are described in 
Table~\ref{tab:gazetteer}. These results also provide a baseline with which to compare
other methods of sequence labeling in the Researcher Name dataset. 
...
for "Ann Smith", it may partially overlap an entry in the dataset for "Mary Ann Smith".
...
the {CoNLL-2003} training set. The {CoNLL-2003} dataset was relabeled so that classifiers only have to extract
...
test set and the Researcher Name test set.
In Table~\ref{tab:conll_to_ner_on_html}, we present the results for a first order \gls{hmm}, 
a Linear Chain \gls{crf}, and a LSTM-CRF classifier with GloVe embeddings~\cite{Huang2015} in this
setting.
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ lllllll }
      \toprule
      \multirow{2}{*}{Model} & \multicolumn{3}{c}{CoNLL-2003} & \multicolumn{3}{c}{Researcher Name} \\
  ...
  \caption{
    ...
    the CoNLL-2003 and Researcher Name test sets.
  }
  \label{tab:conll_to_ner_on_html}
\end{table}

The very low $ F_1 $-score obtained by all models in the Researcher Name test set is worse than the established
...
extraction tasks. The LSTM-CRF model obtained a 0.269 $ F_1 $-score in the Researcher Name test
...
trained with the CoNLL-2003 training set. 

Lets consider some differences between both datasets. The names in the Researcher Name dataset 
...
The number of documents in the Researcher Name dataset is much smaller: only 145 against 
1393. However, the number of sentences in the Researcher Name
dataset is higher: 43870 against 22137. Also, the number of tokens in the Researcher Name dataset
...
to the text from news corpora (\~3.46 words per sentence in the Researcher Name dataset
...
use by sequence models so we should seek other sources of evidence in addition to the text, 
...
signs are frequently present in proper names, as in Mary B., Susan (Susie) Williams, John Al-Azzawi, etc.
Also, they can be useful to detect boundaries between named entities, so their distributions are very relevant 
to this problem.
The {CoNLL-2003} English dataset contains a more generic selection of terms whereas the 
subject of the Researcher Name dataset becomes evident with words such as "professor" and "university" 
happening with a high frequency in the corpus. Also, punctuation signs are much more frequent in
the Researcher Name dataset. Lastly, Figure~\ref{fig:word_frequency_plot}
plots the word frequencies for the hundred most frequent words in the {CoNLL-2003} and 
the Researcher Name datasets. In both plots we have a few very frequent words and a long tail of
...
The probabilities attributed to unseen tokens is another aspect where models will vary significantly 
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ llll }
      \toprule
      \multicolumn{2}{c}{CoNLL-2003} & \multicolumn{2}{c}{Researcher Name} \\
    ...
    \end{tabular}
  \end{center}
  \caption{Ten most frequent words for the {CoNLL-2003} dataset and the Researcher Name dataset.}
...
\begin{figure}[h!]
  \begin{center}
    \begin{tikzpicture}
      ...
      \legend{CoNLL-2003, Researcher Name};
       
      \end{axis}
    \end{tikzpicture}
    \caption{
      Word frequencies plot for the {CoNLL-2003} dataset and the Researcher Name dataset.
    }
...
the Researcher Name dataset was evaluated according to their Precision (P), Recall (R) and F-scores
...
increase. That is why we considered that this approach was not preferable.



\section{Summary}

In this chapter, we introduced the Researcher Name dataset and the Researcher Name Extraction
...
compared the Researcher Name dataset with the popular {CoNLL-2003} English dataset, showing why
a new NER dataset is necessary to adequately solve the researcher name extraction task and 
