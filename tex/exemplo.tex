...
%
Chapter 5 (Sequence Labeling on HTML) describes variations and
improvements to the models described in Chapter 4 to handle \gls{ner}
on HTML documents.
%
Chapter 6 (Researcher Name Extraction Dataset) presents the \gls{rne}
Dataset that concerns the \gls{ner} on HTML task of extracting research names from
faculty directories.
%
Chapter 7 (Experiments) presents the results of multiple experiments
...
a finite set $ L = \{ B-PER, I-PER, ..., O \} $, with both sequences generated by an unknown 
probability distribution. Considering that we observe the sequence of words $ x $ but 
we do not observe the sequence of labels $ y $ is not, then the goal of a classifier is to find: 
%
\begin{equation}
\label{eq:1}
...
What the Markov Assumption implies is that, at any position in the sequenec, the entire history 
...
Further, besides the Markov Assumption, we make the Time Invariance Assumption:
%
\begin{equation}
...
attribute the same probability to every state at time $ t=0 $. For convenience, consider that $ P(Y_1|Y_0) $ 
is the probability of $ Y_1 $ given the initial vector of uniform state
probabilities $ \rho_0 $. 

Now we know how to calculate the state probabilities for a Markov Chain at any timestep
...
The usual choice is to obtain $ \theta $ such that it maximizes the probability of reproducing 
...
from a finite set of states $ L = \{ L_1, L_2, \ldots, L_k \} $. We can write the likelihood as:
%
\begin{equation}
...
the estimates for $ \theta $ and $ \mu $ atributing some probability mass to unseen tokens:
%
\begin{align}
...
%
\begin{equation}
H(Y|X) = - \sum_{i = 1}^{n} P(x, y) \cdot log \left( P(y|x) \right)
\end{equation}
%
It is usual to approximate $ P(x, y) $ with $ P(y|x) \tilde{P}(x) $~\citep{Ratnaparkhi1998}, because
the model probability $ P(x) $ is hard to obtain since its probability space 
is the entire range of feature configurations, which is usually very big.
However, the observed probability $ \tilde{P}(x) $ is defined over a fixed training
sample, so we only need to calculate probabilities for feature configurations that
...
in a page, that does not mean this is the case for most webpages. Consider for example a faculty webpage 
