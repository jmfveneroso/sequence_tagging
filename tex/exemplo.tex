...
from similar webpages, for example in price comparison websites. However,
when a particular extraction task requires more flexibility, such as in cases where
...
\textcolor{red}{
\gls{ner} on the Web can vary significantly depending on the end goal, so systems that perform
...
webpage, this will also be the case on other webpages. HTML feature probabilities cannot be 
generalized because of the great variation in webpage layout. For example, another page may be 
formatted in such a way that tokens inside a list are never named entities, and all tokens 
inside tables are named entities, therefore, HTML feature probabilities calculated outside
the scope of the page are likely to lead to bad predictions.
}
...
is no available counterfactual to confirm the claim that the RNE dataset is in fact a
...
obstacle in the effort of drawing general conclusions.
}

\textcolor{red}{
Some facts may contribute to ascertain the validity of the \gls{rne} dataset as a representative
sample of faculty directories. It contains webpages from 145 universities 
from 42 countries. This amounts to rougly 1.5\% of the total number of universities listed in the
Universities Worldwide Database.\footnote{https://univ.cc/world.php} 
Each country contributes to the dataset roughly in proportion to the number 
of universities in that country relative to the total number. In Table~\ref{tab:countries},
...
possible as training progresses. If the training is kept going indefinitely, the model will probably
...
in the case of the \gls{rne} dataset, the risk of tarnished results increases due to the
...
folds in comparison to the models tested using only the test set, but the variant 
using Group B features still achieves top performance between the three variants. As was
the case for the HMMs, CRFs present a significant standard deviation, what prevents
strong affirmations about the relative performances of different model variations.
}
...
did not change tremendously when compared to the results presented using only the test set. 
All variants privileged recall over precision and the change in the value of $\alpha$ did 
not impact the end result significantly. The best F-$\alpha$ was achieved by the variant
that optimized the F-score with $ \alpha=0.3 $ in contrast with the runs in the test set only, 
where the best model optimized the F-score with $ \alpha=0.5 $. 
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ lllll }
      \toprule
      $\alpha$ & P & R & F-$\alpha$ (CV) & F-$\alpha$ (test set) \\
      \midrule
