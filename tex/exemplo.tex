...
regard to the levels of abstraction learned by the classifiers. While a conventional feed-forward
...
dictionaries containing many instances of the relevant entities. Although, as we argued earlier,
entity instances will rather frequently be unique, rendering dictionary matching approaches
...
Since NER datasets built from news corpora such as CoNLL-2003 cannot provide an adequate 
measure for the performance of sequence models on HTML. We introduce a novel dataset that is 
described in detail in Chapter~\ref{cha:dataset} to run experiments on sequence modelling for 
HTML.
...
punctuation. Our approach to sentence segmentation is described in detail in
...
of researcher name extraction), a good annotator that is able to work 
...
set, as was done both in the MUC conferences and CONLL-2003. Precision accounts for the 
percentage of named entities found by the model that are correct, this is the proportion of 
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ llllllll }
      \toprule
      Data file & Documents & Sentences & Tokens & PER & LOC & ORG & MISC \\
      \midrule
      Training    & 946  & 14987 & 203621 & 1 & 1 & 1 & 1 \\  
      Validation  & 216  & 3466  & 51362 & 1 & 1 & 1 & 1  \\
      Test        & 231  & 3684  & 46435 & 1 & 1 & 1 & 1  \\
      \midrule
      Total       & 1393 & 22137 & 301418 & 1 & 1 & 1 & 1 \\
      \bottomrule
    \end{tabular}
  \end{center}
  \caption{Number of HTML pages, sentences and tokens in each data file}
...
The NER-on-HTML dataset is comparable in size to other popular NER datasets. Its characteristics
are presented in Table~\ref{tab:dataset}. For comparison, the characteristics of the
CONLL-2003 dataset are described in Table~\ref{tab:conll}. Take note that the number of documents
in the NER-on-HTML dataset is much smaller, only 145, against 1393 documents in CONLL-2003. However, 
the number of sentences in the NER-on-HTML dataset is higher: 43870 against 22137. Also, the number 
of tokens in the NER-on-HTML is roughly half the number present in the CONLL-2003 dataset. These 
numbers show that the target HTML documents of the researcher extraction task are longer than news 
texts and, more importantly, they are composed of shorter sentences when compared to the text from 
news corpora (~3.46 words per sentence in NER-on-HTML versus ~13.62 words per sentence in Conll-2003). 
This attests to the fact that HTML provides far less context to be made use by sequence models 
than plain text.

The dataset was divided in training, validation and test sets. Table \ref{tab:dataset} contains
a description of the data files. The validation set was used in the early stopping validation strategy
to avoid overfitting the models, and model performance was evaluated by comparing results in the test 
set. 
...
Thirteen categorical features were associated with each token in the dataset. 
These features were selected through feature engineering from a larger pool
features by eliminating features that did not contribute to the classification 
strategy. The features are presented in Table \ref{tab:features}.
