...
\ppgccufmg{
  ...
  beforetoc={\printglossary[type=\acronymtype]}
}
...
colossal flood of data, each for their own reasons. The web is a vast 
repository of knowledge, but meaningful information gets lost over torrents of unimportant 
data and to extract structured information from this haystack we need
...
webpage patterns such as listings and tables to build wrappers, that are simple programs aimed at extracting relevant 
...
The problem of researcher name extraction can be solved with machine learning methods of 
...
\gls{ner} on the web by evaluating their performance in the researcher 
name extraction task. With this purpose in mind, we introduce a novel \gls{ner} dataset 
...
name extraction. By reliably detecting named entities on the web, we can boost the performance 
...
that, to perform the researcher name extraction task, we need a \gls{wde} method that
...
from our experiments concerning researcher name extraction.
Therefore, when possible, it is desirable that the models be simple to tune for specific extraction 
tasks. Taking this in consideration, we want to find 
the best \gls{ner} methods for a specific task, but trying to derive general conclusions
...
in terms of their Precision, Recall, and F-scores in the researcher name extraction task,
...
\begin{itemize}
\item \textbf{Labeled Researcher Name Extraction dataset:} we introduce a labeled \gls{ner}
...
\item \textbf{Comparison of models for \gls{ner} on the web:} we compare the 
performance of multiple machine learning approaches to \gls{ner} on the web, describing their
relative advantages and disadvantages in terms of quantitative measures such as the F-measure
and qualitative aspects such as complexity and difficulty of training.

\item \textbf{Self-training strategy for Hidden Markov Models:}
we propose a self-training strategy to improve the performance of Hidden Markov Models 
on HTML \gls{ner}.

\item \textbf{Attention mechanism for Neural Network sequence taggers:}
we propose an attention mechanism for improving the performance of Neural Networks 
on HTML \gls{ner}.

\item \textbf{F-measure optimization for Neural Networks:} We propose 
...
\item \textbf{Chapter 3: Related Work} describes the literature concerning 
...
improvements to the models described in Chapter 4 to handle NER
on HTML documents.

\item \textbf{Chapter 6: Researcher Name Dataset} presents the Researcher Name
Dataset that concerns the NER on HTML task of extracting research names from
faculty directories.

\item \textbf{Chapter 7: Experiments} presents the results of multiple experiments
...
a simple ontology from web pages generated with the same template (e.g. collecting house prices 
...
underlying presentation graph (i.e. the DOM tree), the problem of \gls{ie}
...
however, is not relevant to the problem of researcher name extraction because we only
...
data. Family history is a critical piece of information in the decision process for diagnosis and 
...
\gls{wde} is different from \gls{ie} in plain text because HTML documents frequently
...
\gls{ner} is a task in Natural Language Processing that
...
\gls{ner} in the web by assessing their performance in the researcher name extraction task.
...
frequently be unique, rendering dictionary matching approaches ineffective. Therefore, 
...
In Chapter~\ref{cha:dataset}, we introduce the researcher name dataset to evaluate the 
performance of different sequence models in the researcher name extraction task.

Lastly, another important related task in the context of researcher name extraction is performing named 
...
problem of researcher name extraction, which is the main topic of this dissertation, 
...
knowledge from both fields of study, we can get a more robust approach to researcher 
name extraction.
...
Sequence labeling for NER and Part-of-Speech (POS) tagging (labeling nouns, verbs, pronouns, etc.) 
...
Extraction and NER as in the works by~\cite{Bikel1999}, 
...
record for the task at a given time, but some models that held the best F1-score at a given time
...
from great when we consider the researcher name extraction task, being surpassed by much simpler
methods.


\chapter{Techniques for Sequence Labeling}

Many tasks in \gls{nlp}, such as POS tagging and \gls{ner}, can be 
solved by attributing labels to sequences of words. For example, in \gls{ner}, we 
want to classify words into a set of predefined
...
probability distribution. Considering that the sequence of words $ X $ is observed
but the sequence of labels $ Y $ is not, then the goal of a classifier is to find: 
%
\begin{equation}
\label{eq:1}
Y^* = \argmax_{Y} P(Y|X)
...
$ Y $ once we have observed the sequence of words $ X $. For example, in a \gls{ner} 
...
is usually inefficient due to the exponential increase in the number of word and label combinations that 
...
word, then there are already 27 billion combinations for sequences of only three words.
Also, sequences that were not seen during training will always have zero probability. What is problematic, since 
many word sequences that we care about will belong to this category, because proper names are often
unique in a corpus or at least very infrequent (e.g. Hochreiter, Veneroso, Donald, etc.).
...
predictions about the current word's label. The related task of constructing language models consists of predicting the next 
word in a sequence by looking at it's history. A language model that looks at the two previous words 
word is called a bigram model and the model that looks at the three previous words is called a trigram model.
%
\begin{quote}
...
or classes (e.g. PER, ORG, LOC, etc.). First, let's consider a Markov Chain and then
...
When $ k = 1, k = 2, \ldots $ the Markov Chain is said to be of first order, second order, and so on.
What the Markov Assumption implies is that, at any time, the entire history of observed states
...
(before the $ k $ previous states) are independent. 
...
However, in all these cases it is a simplification of reality that makes sequences mathematically 
...
the text that would not be captured by a Markov Model of second or third order. We could consider
employing a Markov Model of higher order to overcome this limitation, however,
...
Nevertheless, HMMs up to third-order can work well despite this limitation if 
...
$ P(y_i=l_b|y_{i-1}=l_a) $ besides the Markov Assumption. The Time Invariance Assumption 
states that: 
%
\begin{equation}
...
\end{equation}
%
With the Time Invariance Assumption, all parameters in our model can be described 
...
attribute the same probability to every state at time $ t=0 $. Also, consider that $ P(y_1|y_0) $ 
...
So far, we have assumed that all states/labels $ Y $ are observable, but in \gls{ner}, only the words 
...
With the \gls{hmm}, we want to predict a sequence of labels $ Y = \{y_1, y_2, ..., y_n\} $ 
(i.e. the Markov Chain) from a sequence of observed states $ X = \{x_1, x_2, ..., x_n\} $
(i.e. the words in a text, or a feature vectors with independent features). Contrastingly,
when we were discussing Markov Chains alone, only the labels were being considered without 
mention of the accompanying words. To incorporate words, we make another assumption:
%
\begin{equation}
...
labels, because it has to assume feature independence. For this reason, other models 
such as \gls{crf}s (explored in the next section) try to overcome this issue by making different 
assumptions, but they lose the closed-form expression for the Maximum Likelihood Estimator, having 
to resort to numerical optimization methods.
...
smoothed estimates~\citep{Manning1999}:
%
\begin{align}
...
probability $ P(Y|X) $ of each label sequence, given the observed sequence $ X $. But, instead of 
...
while HMMs, being generative models, estimate the joint probability. In this section,
...
a vector of observations that expresses features (useful pieces of evidence) such as: the last word 
...
Linear Chain \gls{crf} is defined by:
...
algorithm, as was the case for HMMs. Our analysis only discussed Linear Chain \gls{crf}s. For a thorough 
...
by minimizing the cross entropy. However, once we add multiple layers on top of
...
finding a local optimum solves the parameter estimation problem sufficiently well. Also, methods 
such as adding momentum to gradients can help the optimization algorithm avoid being 
trapped~\citep{Sutskever2013}.
...
iterations. The input vector $ x_t $ for the \gls{rnn} could consist of word features similar to 
the ones used in the previously discussed statistical models. However, word embeddings provide 
better input vectors for language tasks, as we will be discussing in Section~\ref{ssec:word_embeddings}.
...
The timesteps in the context of \gls{lstm}s for sequence labeling refer to position in
...
the output to overcome this obstacle. The main benefit of adding a CRF layer 
in the neural sequence model is that the labels are jointly decoded for a whole sentence 
instead of being predicted individually. Some predicted tags should be highly correlated 
in a named entity recognition task (e.g. I-PER is very likely to happen after B-PER), 
...
is the choice of word embeddings~\citep{Li2018}. Word embeddings are lower-dimensional representations of words in
...
Pre-trained word embeddings are a powerful form of transfer learning, a group of techniques concerned 
with bootstrapping the learning of model parameters by obtaining information from another dataset not directly
related to the problem. 
This is especially useful in \gls{ner}, because named entities that 
...
Next, we describe two methods to build character representations. Both of them receive 
...
of our models. In Section~\ref{sec:self_training}, we propose the self-training strategy
for \gls{hmm}s to incorporate HTML features in the original formulation. 
In Section~\ref{sec:attention_models}, we propose two attention models
to incorporate HTML structural information in our neural sequence taggers. And in
Section~\ref{sec:fscore_optimization}, we propose a new optimization objective for neural 
networks trained for sequence labeling tasks that can make prioritize Recall over Precision,
what can be useful in a name extraction task in the web, because it is easier to filter
out false positive from a huge corpus than to manually find the missed entries.
...
for the sequence labeling task is a generative classifier that has the form:
\begin{equation}
...
and predict the final labels with the whole set of features $ F^T \cup F^H $. If we used the whole
set of features from the start, the HTML features would probably capture wrong patterns as we argued
before. This self-training strategy can be divided in the following steps:
\begin{itemize}
...
\begin{figure}
  ...
  \caption{Example of a webpage snippet with labels attributed by a \gls{hmm}.}
...
Section~\ref{ssec:experiment_char_representations}, we propose to extract HTML structural features with
...
usually evaluated according to their attained results in terms of Precision, Recall, or their harmonic 
mean (i.e. the F-measure).
...
models for sequence labeling. The F-measure function can be calculated in terms of a triple (A,B,C):
...
context when making label predictions, what can be very useful when we cannot extract so much
information from the textual context, as is the case in many webpages (especially listings and tables).
...
goals. We argued that Recall is slightly more important than Precision in web extraction tasks, therefore
...
between plain text and HTML, as we will discuss later in this chapter. The lack of other \gls{ner} 
...
experimentally that fifty tokens provide a large enough context to be made use by sequence 
taggers and allows efficient training using a GPU. A few sentences in the dataset would be 
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Description of the data files in the \gls{rne} dataset.}
  \label{tab:dataset}
\end{table}
...
The choice for $ \beta $ depends on the relative importance attributed to Precision over Recall.
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{DBLP dictionary coverage in each data file of the \gls{rne} dataset.}
  \label{tab:gazetteer}
\end{table}
...
a Linear Chain \gls{crf}, and a LSTM-CRF classifier with GloVe embeddings~\citep{Huang2015} in the 
...
extraction tasks. The LSTM-CRF model obtained a 0.269 $ F_1 $-score in the \gls{rne} test
set, what is surprising considering that it is a very efficient approach to sequence labeling in plain 
text. By looking at some of its mistakes we can shed some light on this result. The LSTM-CRF model
...
account for some of the mistakes of the LSTM-CRF model.
...
the \gls{rne} dataset. Lastly, Figure~\ref{fig:word_frequency_plot}
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Ten most frequent words for the {CoNLL-2003} dataset and the \gls{rne} dataset.}
...
\begin{figure}[h!]
  \begin{center}
    ...
    \end{tikzpicture}
    \caption{
      Word frequencies plot for the {CoNLL-2003} dataset and the \gls{rne} dataset.
    }
...
order in the test sets of the Researcher Name dataset. To allow comparison with the baselines, we also 
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{
      ...
      features besides the current word on the test set of the Researcher Name dataset.
    }
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Features used in the Researcher Name dataset}
  \label{tab:features}
\end{table}
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{
      ...
      Linear Chain CRFs without features besides the current word on the test set of the Researcher Name dataset.
    }
...
Figure~\ref{fig:fscore_optimization} presents the results on the test set of the Researcher Name dataset
...
CRFs and Neural Networks in the Researcher Name dataset.
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Overview of the best models for the name extraction task in the Researcher Name dataset.}
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Overview of the best models for the name extraction task in the Researcher name test set after the filtering strategy.}
