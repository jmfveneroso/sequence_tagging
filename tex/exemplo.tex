...
need only estimate the parameters $ \hat{P}(f_{j}|y_a) $, relative to each possible
assignment of $ y_a $.
...
%
\begin{equation}
\hat{P}(f_{j}=1|y_a) = \frac{\text{Count}(f_{j}=1, y_a)}
{\text{Count}(f_{j}=0, y_a) + \text{Count}(f_{j}=1, y_a)}
\label{eq:feature_mle}
\end{equation}
%
Where $ \text{Count}(f_{j}=1, y_a) $ is the number of times in the training set that
feature $ f_j $ was equal to one when the actual label was $ y_a $ (the other counts follow the same logic).
...
%
\begin{equation}
\hat{P}(f_{j}=1|y_a) = \frac{\text{Count}(f_{j}=1, \tilde{y}_a)}
{\text{Count}(f_{j}=0, \tilde{y}_a) + \text{Count}(f_{j}=1, \tilde{y}_a)}
\end{equation}
%
Where $ \tilde{y}_a $ is the label predicted by the \gls{hmm} trained with only textual features.
...
the ones used in the previously discussed statistical models. However, word embeddings usually provide 
...
their values tend to get vanishingly small, what makes training less effective.
...
In addition to the memory cell $ c $, the LSTM incorporates three gates to control 
the flow of information that comes in and out of this cell.
...
The equations for the LSTM unit are:
\begin{align*}
...
often obtained with unsupervised clustering methods for words in large corpora. They are lower-dimensional in
contrast to the size of the vocabulary, which usually consists of a few hundred thousand words. If we were to use 
...
different words according to their shared features, for example using cosine similarity. And lastly, word embeddings 
...
Bi-LSTM-CRF achieved an 0.902 $F_1$-score, while 
...
running on an AWS GPU instance took approximately 4412 seconds to train 
...
In the previous chapters, we discussed sequence models that can be generally employed in 
...
\gls{hmm}s, we propose two methods to boost the performance of neural networks in
sequence labeling on the Web.
...
by posing some additional research questions:
%
\begin{enumerate}
...
test how difficult it would be to filter out false positives from the results. It consists of automatically 
