...
maximize the F-measure in terms of expected utility. We take the method proposed by Jasche to 
approximately maximize the F-measure of a classifier based on a logistical regression 
model and apply it to our neural architectures. Then, the optimization objective becomes:

\begin{equation}
F_\alpha(\theta) = \frac{A(\theta)}{\alpha n_{pos} + (1-\alpha) m_{pos}(\theta)}
\end{equation}

Which resembles the F-measure in terms of a triple (A,B,C):

\begin{equation}
F_\alpha(A, B, C) = \frac{A}{A + \alpha B + (1-\alpha) C}
\end{equation}

that is equivalent to the $ \alpha $-weighted harmonic mean defined in (CITE FSCORE FORMULA HERE).
A is the number of true positives, B is the numberof false negatives, and C is the number of false
positives.

WE MAKE A SIMPLIFICATION THAT EACH LABEL IS A EITHER POSITIVE OR NEGATIVE INSTEAD OF GROUPS OF LABELS.
