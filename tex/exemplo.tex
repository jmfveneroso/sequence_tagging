...
solved by attributing labels to sequences of words. In \gls{ner} particularly, we 
...
task in \gls{nlp} can be modeled generically by considering that $ X = \{x_1, x_2, ..., x_n\} $ 
...
Also, sequences that were not seen during training will always have zero probability. What is problematic, 
because proper names are often unique or very infrequent in a corpus (e.g. Veneroso, Schmidt, etc.),
...
A related task in \gls{nlp} called Language Modelling consists of predicting the next 
word in a sequence by looking at the sequence's history. Using the Markov Assumption, 
...
We have not yet made any restrictions on the form taken by the transition probabilities 
...
$ \hat\theta_{a, b} = \infty, \; \forall \; a \in L, b \in L $. This happens because we did not
...
$ b $ in sequence $ Y $. A value that can be easily calculated with a labeled dataset.
...
mention of the accompanying words. With the \gls{hmm}, we want to predict a sequence of 
labels $ Y = \{y_1, y_2, ..., y_n\} $ 
...
try to overcome this issue by making different assumptions. 
...
\end{equation}
%
This is the main equation of the \gls{hmm}.
...
usually too big, while the empirical prior $ \tilde{P}(x) $ is easy to obtain~\citep{Ratnaparkhi1998}.
...
it becomes difficult due to the vanishing gradient problem~\citep{Bengio1994}. 
...
character embedding size (determined experimentally). 
% Also, the character embeddings are 
...
at the bottom of Bi-LSTM-CRF to encode character-level information. The \gls{cnn}
...
what can be useful in a name extraction task on the web, because it is easier to filter
...
Similar to the way we extracted morphological features with character representations in
Section~\ref{ssec:char_representations}, we propose to extract HTML structural features with
...
matching approach presented in Chapter~\ref{cha:dataset}.
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ lllllll }
      ...
      Logistic               & 0.952 & 0.171 & 0.122 & 0.116 & 0.156 & 0.133   \\
...
Figure~\ref{fig:hmm_no_features} shows the performance of \textit{HMMs} up to third 
...
the Cramér's V measure~\footnote{
...
were trained with Group A features and Self-Trained with Group C features.
...
and a CRF without features in the \gls{rne} task. 
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{CRFs trained with the features from Group A and Group B.}
...
self-trained HMM-2, it still shows a better overall performance (0.879 F1). 
...
next section. Table~\ref{tab:crf_results} presents the detailed results for CRFs at the \gls{rne}
  ...
  \begin{center}
    \begin{tabular}{ lllllll }
      ...
      \multirow{2}{*}{Model} & \multicolumn{3}{c}{Validation} & \multicolumn{3}{c}{Test} \\
                             ...
                             & \multicolumn{1}{c}{P} & \multicolumn{1}{c}{R} & \multicolumn{1}{c}{F1} \\
      \midrule
      CRF (featureless)      & 0.806 & 0.805 & 0.806 & 0.795 & 0.710 & 0.750 \\
...
representations can boost the performance of Bi-LSTM-CRFs in the same task. In Section~\ref{ssec:experiments_nn_3}, we check if
the choice of pre-trained Word Embeddings alters the results significantly. In Section~\ref{ssec:experiments_nn_4}, we assess the
performance of soft and hard attentions models. Lastly, in Section~\ref{ssec:experiments_nn_5}, we show the results of a Bi-LSTM-CRF model trained
...
We only use GloVe-300 word embeddings as inputs to the Bi-LSTM-CRF model.
...
\begin{figure}[h!]
  \begin{center}
    ...
    \end{tikzpicture}
    \caption{
      Performance of the Bi-LSTM-CRF with only GloVe embeddings in comparison to the featureless HMM-2 and 
      the featureless CRF.
    }
    \label{fig:lstm_crf}
  \end{center}
...
CNN character representations (CNNc) and LSTM character representations (LSTMc) with the plain Bi-LSTM-CRF. The character embeddings had 50 weights,
...
\begin{table}[h]
  ...
  \begin{center}
    \begin{tabular}{ lllllll }
      ...
      +LSTMc                & 0.928 & 0.950 & 0.939 & 0.920 & \textbf{0.886} & \textbf{0.902} \\
...
The Hard Attention layer improved the original model by roughly 0.005 $F_1$-score.
...
embeddings took approximately 4412 seconds to train and run predictions, the same model with ELMo embeddings took
approximately 9437 seconds.
...
usually work by minimizing the Cross Entropy, or maximizing the likelihood. However, what we ultimately care about is the
...
Figure~\ref{fig:fscore_optimization} presents the results on the test set of the \gls{rne} dataset
...
$ \alpha=0.8 $ achieves the best Recall (0.923) and the best $ F_{\alpha} $ (0.905).
...
Bi-LSTM-CRF with a Hard Attention Layer achieved an 0.907 $F_1$-score, while 
the HMM-2 using a Self-Training strategy achieved an 0.879 
F1-score. This is a 0.028 increase, which is not insignificant, but it comes to mind if the additional
...
\begin{table}[h]
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Overview of the complexity and training times of the best models for the RNE task.}
