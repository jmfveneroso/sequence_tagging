...
knowing all the sequence history. Therefore, conditioned on this limited set of previous states, 
...
continuous-time Markov Chains (http://www.columbia.edu/~ks20/stochastic-I/stochastic-I-CTMC.pdf), 
but they are hardly useful on text modelling since words and characters in a text are 
...
Nevertheless, it always represents a simplification of reality to make the sequences mathematically 
treatable. The extent to which this simplification will hurt our predictions depend of course on the 
nature of the studied phenomena. However, it must be said that there are Markov Chain variations that try to
overcome these limitations such as (CITE time-homogeneous MCs, MC with memory), as well as other models
...
sentence: "The wife had discovered that the husband was carrying on an intrigue with a French 
girl, who had been a governess in their family". To understand to which entities the passages 
"governess" and "in their family" relate to, we need to keep track of distant relations in the text
that would not be captured by a window $ k $ of size three or four. Yet, this limitation is of lesser
significance in the \textit{Named Entity Recognition} task, because we can complement the model with
features that make up for the lack of knowledge provided by a longer context. 
In practice, considering more than three or four previous states at any time makes 
parameter esimation unreliable for the reasons explained in the beginning of the chapter, also 
when $ k $ increases, the cost for decoding the optimal sequence of states becomes prohibitively 
high in terms of computation time. And, as we will see later, Markov Chains of second and third order 
are fairly adequate models for sequence labeling tasks. 

Next comes the problem of parameter estimation. At this time we have made no restrictions
on the form taken by the transition probabilities $ P(s_a|s_b) $ from state $ s_a $ to $ s_b $.
Besides the \textit{Markov Assumption}, it is usual to consider them to be time invariant. That is:

\begin{equation}
P(y_{i+1} = s_a |y_{i} = s_b) = P(y_i = s_a |y_{i-1} = s_b)
\end{equation}

Given states $ s_a $ and $ s_b $, the probability $ P(y_{i+1} = s_a |y_{i} = s_b) $ of
going from $ s_a $ to $ s_b $ on timestep $ i $ is the same as in any other timestep.
With that statement, all parameters in our model can be described by a transition 
matrix of size $ k \times k $ where the row $ i $ holds the probabilities of going from state
$ s_i $ to the states $ s_j $ represented by each column index $ j $, including itself.
Naturally, the probabilities of each row $ i $ must sum up to one since they represent
the entire scope of transition possibilities starting from state $ S_i $.
For example, Equation \ref{eq:matrix} describes a $ 2 \times 2 $ transition matrix $ T' $ 
with states $ {T'_0, T'_1} $ where there is a 0.3 chance of staying in $ T'_0 $ if we 
start in $ T'_0 $ and a 0.7 chance of going from $ T'_0 $ to $ T'_1 $. Likewise there
is a 0.6 chance of staying in $ T'_1 $ if we start in $ T'_1 $ and a 0.4 chance 
of going from $ T'_1 $ to $ T'_0 $ in any timestep.
...
If vector $ v_i $ of size $ k \times 1 $ represents the probabilities of being in each 
state at a given timestep $ i $, then: 

\begin{equation}
v_{i+1} = v_i \cdot \theta
\end{equation}

is the probability of being in each state at the next timestep. And, for an arbitrary
number of timesteps $ t $:

\begin{equation}
v_{t} = v_0 \cdot \theta^t
\end{equation}

DESCRIBE STARTING PROBABILITIES FOR HIGHER ORDER MMs 

Where $ v_t $ is a $ k \times 1 $ vector with the probabilities of being in each state
at timestep $ t $ and $ v_0 $ is the starting probability of being in each state. If we
know nothing about the initial conditions we can simply attribute the same probability 
to every state. Now, given a transition matrix $ \theta $, to obtain the likelihood 
$ \mathcal{L(Y; \theta)} $ of a sequence of observed states $ Y = \{ y_1, y_2, ..., y_n \} $,
we need only calculate the product of all transitions, which are known from the data:

\begin{equation}
P(Y) = P(y_0) \prod_{i=1}^{n} P(y_i|y_{i-1}) \\
\mathcal{L}(Y; \theta) = v_0 \prod_{i=1}^{n} \theta_{y_i, y_{i-1}}
\end{equation}

We can write $ \mathcal{L}(Y; \theta) $ in another form by defining $ \eta_{a,b} $
to be the count of transitions from $ a $ to $ b $ in the sequence $ X $. This way:

\begin{equation}
\mathcal{L}(Y; \theta) = v_0 \prod_{a, b} \theta_{a, b}^{\eta_{a, b}}
\label{eq:hmm_likelihood}
\end{equation}

However, this expression shows only half the truth, because not all the parameters
$ \theta_{a, b} $ are free since every row in the transition matrix must sum up to one.
To introduce this constraint in the likelihood calculation, we must choose a column
$ B $ arbitrarily for which the values $ \theta_{a, b=B} $ will be determined by the
remaining values in each corresponding row. This way, by choosing $ B=0 $ 
Equation~\ref{eq:hmm_likelihood} becomes:

\begin{equation}
\begin{array}{l}
\mathcal{L}(Y; \theta) = v_0 \prod_{a, b \neq 0} \theta_{a, b}^{\eta_{a, b}} 
\prod_{a, b = 0} (1 - \sum_{b' \neq 0} \theta_{a, b'})^{\eta_{a, b}} \\
\mathcal{l}(Y; \theta) = ln v_0 \sum{a, b \neq 0} \eta_{a, b} ln(\theta_{a, b}) +
\sum{a, b = 0} \eta_{a, b} ln(1 - \sum_{b' \neq 0} \theta_{a, b'})
\end{array}
\label{eq:hmm_likelihood}
\end{equation}

The likelihood function $ \mathcal{L}(Y; \theta) $ is always positive, so finding the parameters 
that maximize $ \mathcal{l}(Y; \theta) $ is identical to maximizing the likelihood itself, with the
benefit of transforming a product over transition probabilities into a sum of logarithms. Next, we must
calculate the derivatives for the log likelihood in order to find the maximum:

\begin{equation}
\begin{array}{l}
\frac{\partial \mathcal{l}(Y; \theta)}{\partial \theta_{a, b}} = 
\frac{\eta_{a, b}}{\theta_{a, b}} - \frac{\eta_{a, 0}}{1 - \sum_{b' \neq 0} \theta_{a, b'}} = 0 \\
\frac{\eta_{a, b}}{\theta_{a, b}} = \frac{\eta_{a, 0}}{1 - \sum_{b' \neq 0} \theta_{a, b'}} \\
\frac{\theta_{a, b}}{1 - \sum_{b' \neq 0} \theta_{a, b'}} = \frac{\eta_{a, b}}{\eta_{a, 0}}
\end{array}
\end{equation}

Therefore, knowing that $ \eta_{a, b} \propto \theta_{a, b} $:

\begin{equation}
\theta_{a, b} = \frac{\eta_{a, b}}{\sum_{b'}\eta_{a, b'}}
\end{equation}

% By making $ \frac{\partial\mathcal{l}(Y; \theta)}{\theta_{a, b}} = 0 $, we can find the maximum value
% for $ \mathcal{l}(Y; \theta) $. To ascertain that this is a global maximum we take the second 
% derivative:
% 
% \begin{equation}
% \frac{\partial^2 \mathcal{l}(Y; \theta)}{\partial \theta_{a, b}^2} = 
% \frac{-K(a, b, X)}{\theta_{a, b}^2} - \frac{n - K(a, b, X)}{(1 - \theta_{a, b})^2}
% \end{equation}
% 
% Observe that the partial second derivatives are always negative, what makes this function strictly 
% concave, therefore if there is a maximum value, it is a global maximum. Finally:
% 
% \begin{equation}
% \begin{array}{l}
% \frac{\partial \mathcal{n}(Y; \theta)}{\partial \theta_{a, b}} = 0 \\
% \frac{K(a, b, X)}{\theta_{a, b}} - \frac{n - K(a, b, X)}{1 - \theta_{a, b}} = 0 \\
% theta_{a, b} = \frac{K(a, b, X)}{n}
% \end{array}
% \end{equation}

We get a closed form expression for the maximum likelihood estimator $ theta_{a, b} $.
What is really convenient, since this is the average number of transitions from state $ a $ to state
$ b $ in sequence $ X $. A value that can be easily obtained with $ O(n) $ time complexity. So
far, we assumed all states $ X $ are observable, but in NER, only the words 
are observed, while the Named Entity labels associated with these words are not. That is why we need
another layer of complexity. The Hidden Markov Model (HMM) differs from the Markov Chain in that it 
does not observe the states $ X $ directly, but rather a probabilistic function of these states. 

With the Hidden Markov Model we want to predict a sequence of labels $ Y = \{y_1, y_2, ..., y_n\} $ 
(i.e. our Markov Chain) from a sequence of observed states $ X = \{x_1, x_2, ..., x_n\} $
(i.e. the words in the text). So we make an additional assumption:

\begin{equation}
P(x_i|y_{i-1}x_{i-1}, ..., y_1x_1) = P(x_i|y_i)
\end{equation}

That is, the probability of observing the word $ x_i $ depends only on the current label
$ y_i $, or hidden state (e.g. PER, LOC, ORG, etc.). So, by making this assumption, we are
essentialy stating that if we know a token's assigned label, we can reliably predict what are the 
probabilities that this token takes any value in a vocabulary. This is, of course, a very
simplistic assumption and many models (such as CRFs) try to overcome this issue, but this 
step is necessary if we want to get a closed form estimator for our generative model (IS THAT SO?). 
To model the observation probability distributions $ P(x_i|y_i) $, we assume again that 
the probabilities are time invariant and that $ x_i $ takes its value from a fixed vocabulary 
with size $ V $. We introduce an \textit{Emission Matrix} $ V \times K $ to hold these 
probabilities where each cell $ V_{i,j} $ represents the probability that, given the $ Y_i $,
we observed the word $ X_j $. Now, to calculate the probability of observing a vector of labels 
$ Y $ given a sequence of observations $ X $ can be calculated with Bayes' theorem:

\begin{equation}
P(Y|X) = \frac{P(X|Y) P(Y)}{P(X)}
\end{equation}

And we get the conditional likelihood:

\begin{equation}
\mathcal{L}(Y|X; \theta) = v_0 \prod_{a, b} \theta_{a, b}^{\eta_{a, b}} \prod_{b, c} \psi_{b, c}^{\mu_{b, c}}
\mathcal{l}(Y|X; \theta) = ln v_0 \sum_{a, b} \theta_{a, b} ln(\eta_{a, b}) \sum_{b, c} \mu_{b, c} ln(\psi_{b, c})
\end{equation}

Parameters $ \theta $ and $ \psi $ are independent, so the procedure to find parameters $ \theta_{a,b} $ 
remains the same and we can employ the same procedure used on (REFERENCE) to find parameters $ \psi $,
what yields:

\begin{equation}
\psi_{b, c} = \frac{\mu_{a, b}}{\sum_{c'}\mu_{b, c'}}
\end{equation}

That is the expected number of times we observed word $ X $ when we were at state $ Y $. A number
that is also very easy to obtain in $ O(n) $ time. With that we conclude the parameter estimation
for the Hidden Markov Model when have access to labeled data. If we only have unlabeled data we
can do the parameter estimation with the Baum-Welch algorithm, though the results are much less
reliable on the sequence labelling task.

The last task we need to do is calculating the most probable sequence of labels given a sequence of
observations. To obtain this sequence we employ the Viterbi algorithm, which is explained in Appendix
